...
//EN_TIMER_ERROR_T TIMER_delay_ms(Uint16_t u16_a_interval) {
    //if((u8_g_TMRShutdownFlag != STR_NULL && *u8_g_TMRShutdownFlag == 1)) return TIMER_ERROR; // sudden break flag
   ///* if ( ( u16_a_interval / SECOND_OPERATOR ) > ( MAX_TMR_DELAY ) ) {
    ...
    //else {
        ...
        //if (d64_a_delay < MAX_DELAY) {
            ...
            //TMR_U8_TCNT0_REG = (Uint8_t) ((MAX_DELAY - d64_a_delay) / TICK_TIME);
...
////            u8_g_TMR0InitialVal = (Uint8_t)(MAX_COUNTS - ((d64_a_delay / TICK_TIME) / u16_g_overflowNumbers));
            //TMR_U8_TCNT0_REG = (Uint8_t) ((MAX_COUNTS) - ((d64_a_delay - (MAX_DELAY * (u16_g_overflowNumbers - 1.0))) /
 ...
 *         (TMR_OK if the operation succeeded, TMR_ERROR otherwise)
 */
/*EN_TMR_ERROR_T TMR_TMR0CleareCompMatInit(Uint8_t u8_a_outCompValue)
{
 ...
 *         (TMR_OK if the operation succeeded, TMR_ERROR otherwise)
 */
/*EN_TMR_ERROR_T TMR_TMR2CleareCompMatInit(Uint8_t u8_a_outCompValue)
{
	if (u8_a_outCompValue > MAX_COUNTS)
	{
		return TMR_ERROR;
